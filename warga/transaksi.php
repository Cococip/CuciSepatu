        ...
        <table class="table table-bordered table-hover table-striped" >
            <thead>
                <tr class="nw">
                    ...
                    <th class="text-center">Dijemput</th>
            ...
            <tbody>
                ...
                WHERE u.id_user = '$id_user' AND p.status_transaksi IN ('pending', 'diproses') AND p.status_pesanan IN ('pending', 'diproses')
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_object()) {
                        ...
                        // Mendapatkan no_metode
                        $metode_id = $row->id_metode;
                        $query_metode = "SELECT no_metode FROM metode WHERE id_metode = '$metode_id'";
                        $result_metode = $db->query($query_metode);
                        $no_metode = $result_metode->fetch_object()->no_metode;
                        ...
                        echo "<td class='text-center'>{$no_metode} - {$row->nama_metode}</td>";
                        ...
                        echo "<td class='text-center'>
                                    <a class='btn btn-sm btn-warning' href='upload.php?id_pesanan={$row->id_pesanan}'>bukti</a> ||
